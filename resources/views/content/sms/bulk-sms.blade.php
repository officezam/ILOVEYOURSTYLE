      ...
      <div class="col-md-12 col-12">
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">Send Message to List</h4>
          </div>
          <div class="card-body">
              <form class="form form-horizontal" action="{{'bulk-sms-send'}}" method="post" accept-charset="UTF-8" enctype="multipart/form-data" >
                <input name="_token" type="hidden" value="{{ csrf_token() }}"  >
              <div class="row">
                <div class="col-12">
                  <div class="form-group row">
                    <div class="col-sm-3 col-form-label">
                      <label for="first-name">Select a Phone List</label>
                    </div>
                    <div class="col-sm-9">
                      <select class="select2 form-control" name="list_id" id="ToPhone">
                        @foreach ($AllList as $product)
                          <option value="{{ $product->id  }}">{{ $product->list_name.' Number of Contacts '.count($AllList)}}</option>
                        @endforeach
                      </select>
                      @error('ToPhone')
